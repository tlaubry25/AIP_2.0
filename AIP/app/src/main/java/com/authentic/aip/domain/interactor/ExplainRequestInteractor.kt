...
class ExplainRequestInteractor @Inject constructor(
    private val repository: AppRepository
){
    operator fun invoke(uid : String, cddeid : String, deli:Int, orderType:Char, comment:String): Flow<Resource<Any>> = flow {
        try {
            emit(Resource.Loading<Any>())
            val any = repository.explainRequest(uid =  uid, cddeid = cddeid, deli = deli, orderType = orderType, comment = comment).data
